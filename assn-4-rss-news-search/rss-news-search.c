...
static const char *const kDefaultFeedsFile = "../assn-4-rss-news-search-data/rss-feeds-tiny.txt";
...
static void BuildIndices(const char *feedsFileName)
{
  ...
  printf("feeds file name is %s", feedsFileName);
  assert(infile != NULL);
  STNew(&st, infile, kNewLineDelimiters, true);
...
static void ProcessFeed(const char *remoteDocumentName)
{
  ...
  switch (urlconn.responseCode) {
      case 0: printf("Unable to connect to \"%s\".  Ignoring...", u.fullName);
              break;
