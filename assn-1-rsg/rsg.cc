...
/**
 * Expand a non-terminal and put the phrases in the output vector.
 */

void expand(string& nonterminal, map<string, Definition>& grammar, vector<string>& outputVec) {
  ...
  for(it = prod.begin(); it != prod.end(); ++it){
    if(grammar.count(*it) == 0) {
      outputVec.push_back(*it);
    }
    else { // it's a non terminal, recurse.
      expand(*it, grammar, outputVec);
    }
  }
}
